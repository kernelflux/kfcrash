...
static KFCrashAppMemoryProvider gMemoryProvider = nil;

static KFCrashAppMemoryProvider KSCrashAppMemoryGetProvider(void)
{
    os_unfair_lock_lock(&gMemoryProviderLock);
    KFCrashAppMemoryProvider provider = gMemoryProvider;
    os_unfair_lock_unlock(&gMemoryProviderLock);
    return provider;
}

FOUNDATION_EXPORT void testsupport_KFCrashAppMemorySetProvider(KFCrashAppMemoryProvider provider)
{
    os_unfair_lock_lock(&gMemoryProviderLock);
...
@interface KFCrashAppMemoryTracker () {
    ...
    uint64_t _footprint;
    KFCrashAppMemoryState _pressure;
    KFCrashAppMemoryState _level;

    // weak objects are `KFCrashAppMemoryTrackerObserverBlock`'s
    NSPointerArray *_observers;
}
@end

@implementation KFCrashAppMemoryTracker

+ (instancetype)sharedInstance
{
    static KFCrashAppMemoryTracker *sTracker;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sTracker = [[KFCrashAppMemoryTracker alloc] init];
        [sTracker start];
    });
    return sTracker;
}
...
- (instancetype)init
{
    if ((self = [super init])) {
        ...
        _heartbeatQueue = dispatch_queue_create_with_target("com.kfcrash.memory.heartbeat", DISPATCH_QUEUE_SERIAL,
...
- (void)start
{
    ...
    __unsafe_unretained __typeof(self) weakMe = self;
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(_appDidFinishLaunching)
                                               ...
                                               object:nil];
#endif
    NSArray<KFCrashAppMemoryTrackerObserverBlock> *observers = nil;
    {
        os_unfair_lock_lock(&_lock);
        [_observers compact];
...
static KFCrashAppMemory *_Nullable _ProvideCrashAppMemory(KFCrashAppMemoryState pressure)
{
    ...
    return [[KFCrashAppMemory alloc] initWithFootprint:info.phys_footprint remaining:remaining pressure:pressure];
}

- (nullable KFCrashAppMemory *)currentAppMemory
{
    KFCrashAppMemoryProvider provider = KSCrashAppMemoryGetProvider();
    return provider ? provider() : _ProvideCrashAppMemory(self.pressure);
}

- (void)_handleMemoryChange:(KFCrashAppMemory *)memory
                       type:(KFCrashAppMemoryTrackerChangeType)changes
                  observers:(NSArray<KFCrashAppMemoryTrackerObserverBlock> *)observers
{
    for (KFCrashAppMemoryTrackerObserverBlock obs in observers) {
        obs(memory, changes);
    }
...
- (void)_heartbeat:(BOOL)sendObservers
{
    ...
    KFCrashAppMemory *memory = [self currentAppMemory];

    KFCrashAppMemoryState newLevel = memory.level;
    uint64_t newFootprint = memory.footprint;

    NSArray<KFCrashAppMemoryTrackerObserverBlock> *observers = nil;
    KFCrashAppMemoryState oldLevel;
    ...
    if (newLevel != oldLevel && sendObservers) {
        ...
        dispatch_once(&onceToken, ^{
            ...
            sSimulatorMemoryKillEnabled = [env[@"KFCRASH_SIM_MEMORY_TERMINATION_ENABLED"] boolValue];
        });
...
- (void)_memoryPressureChanged:(BOOL)sendObservers
{
    ...
    KFCrashAppMemoryState newPressure = KSCrashAppMemoryStateNormal;
    ...
    NSArray<KFCrashAppMemoryTrackerObserverBlock> *observers = nil;
    KFCrashAppMemoryState oldPressure;
    {
        os_unfair_lock_lock(&_lock);
...
- (KFCrashAppMemoryState)pressure
{
    KFCrashAppMemoryState state;
    {
        os_unfair_lock_lock(&_lock);
        state = _pressure;
        os_unfair_lock_unlock(&_lock);
