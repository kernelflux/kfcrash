...
#if OBJC_HAVE_TAGGED_POINTERS // (KFCrash) This line is moved here to make `objc_tag_index_t` enum visible for i386.
...
// (KFCrash) Added to handle runtime checks for Split Tagged Pointers,
...
// (KFCrash) Macros were rewritten to include runtime checks for split tagged pointers, based on objc4-912.3.
...
// (KFCrash) Weakly linked to support *OS versions prior to objc4-818.2, where this symbol may not be available.
...
static inline uintptr_t _objc_basicTagToObfuscatedTag(uintptr_t tag) {
    ...
    // (KFCrash) Fallback: return the original tag if permutations are unavailable or tag is out of range.
...
static inline uintptr_t _objc_obfuscatedTagToBasicTag(uintptr_t tag) {
    ...
    // (KFCrash) Fallback: return the original tag if within range, otherwise return 7.
    // Runtime handles the availability of split tagged pointers, so fallback is typically unnecessary,
    // but it's included as a safeguard.
    return (tag < 8) ? tag : 7;
}
