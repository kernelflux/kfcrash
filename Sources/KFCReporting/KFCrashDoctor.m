...
@implementation KFCrashDoctorParam

@end

@interface KFCrashDoctorFunctionCall : NSObject
...
@implementation KFCrashDoctorFunctionCall
...
- (NSString *)descriptionForObjCCall
{
    ...
    KFCrashDoctorParam *receiverParam = [self.params objectAtIndex:0];
    ...
    KFCrashDoctorParam *selectorParam = [self.params objectAtIndex:1];
    ...
    for (int paramNum = 0; paramNum < paramCount; paramNum++) {
        [string appendString:@":"];
        if (paramNum < 2) {
            KFCrashDoctorParam *param = [self.params objectAtIndex:(NSUInteger)paramNum + 2];
            if (param.value != nil) {
...
- (NSString *)descriptionWithParamCount:(int)paramCount
{
    ...
    for (int i = 0; i < paramCount; i++) {
        KFCrashDoctorParam *param = [self.params objectAtIndex:(NSUInteger)i];
...
- (KFCrashDoctorFunctionCall *)lastFunctionCall:(NSDictionary *)report
{
    KFCrashDoctorFunctionCall *function = [[KFCrashDoctorFunctionCall alloc] init];
    ...
    for (NSString *regName in regNames) {
        KFCrashDoctorParam *param = [[KFCrashDoctorParam alloc] init];
...
- (NSString *)zombieCall:(KFCrashDoctorFunctionCall *)functionCall
{
...
- (NSString *)diagnoseCrash:(NSDictionary *)report
{
    @try {
        ...
        KFCrashDoctorFunctionCall *functionCall = [self lastFunctionCall:report];
        ...
        if (symbols) {
            return [NSString
                stringWithFormat:
                    @"No diagnosis due to exception %@:\n%@\nPlease file a bug report to the KFCrash project.", e,
                    symbols];
        }
        return [NSString
